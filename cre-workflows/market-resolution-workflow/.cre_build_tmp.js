// ../../Users/ASHWIN GOYAL/OneDrive/Desktop/AETHER SENTINEL/cre-workflows/market-resolution-workflow/.cre_build_tmp.js
...
var configSchema = exports_external.object({
  ...
  maxMarketsToCheck: exports_external.number().default(10),
  gasLimit: exports_external.string().default("500000")
});
var predictionMarketAbi = parseAbi([
...
var fetchPriceData = (nodeRuntime) => {
...
var onCronTrigger = (runtime2) => {
  ...
  runtime2.log("=== AETHER SENTINEL Market Resolution Workflow ===");
  runtime2.log(`Market Contract: ${config.predictionMarketAddress}`);
  ...
  runtime2.log("[Step 1] Reading market count from PredictionMarket...");
  let marketCount = 0;
  try {
    ...
    const countRaw = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
    ...
    if (hexData && hexData !== "0x") {
      marketCount = Number(decodeFunctionResult({
        abi: predictionMarketAbi,
        functionName: "marketCount",
        data: hexData
      }));
    } else {
      runtime2.log("  Contract returned empty data — no markets deployed yet");
    }
  } catch {
    runtime2.log("  Could not read market count — contract may not be deployed yet");
  }
  runtime2.log(`  Total markets: ${marketCount}`);
  runtime2.log("[Step 2] Fetching offchain price data from CoinGecko...");
  let ethPrice = 0;
  try {
    const ethPriceCents = runtime2.runInNodeMode(fetchPriceData, consensusMedianAggregation())().result();
    ethPrice = Number(ethPriceCents) / 100;
  } catch {
    runtime2.log("  CoinGecko API unavailable — using fallback price");
    ethPrice = 2500;
  }
  runtime2.log(`  ETH Price: $${ethPrice}`);
  if (marketCount === 0) {
    runtime2.log("No markets found — workflow complete with price data fetched.");
  ...
  runtime2.log("[Step 3] Checking markets for settlement...");
  ...
  for (let i2 = 0;i2 < limit; i2++) {
    try {
      ...
      const marketRaw = evmClient.callContract(runtime2, {
        call: encodeCallMsg({
          from: zeroAddress,
          to: config.predictionMarketAddress,
      ...
      runtime2.log(`  Market #${i2}: expired and unresolved — settling...`);
      const winningOutcome = ethPrice >= 3000 ? 0 : 1;
      const reportData = encodeAbiParameters(parseAbiParameters("uint256 marketId, uint256 winningOutcome, uint256 ethPrice"), [BigInt(i2), BigInt(winningOutcome), BigInt(Math.round(ethPrice * 100))]);
      ...
      runtime2.log(`  Settled market #${i2} — outcome: ${winningOutcome}, tx: ${txHash}`);
      settledCount++;
    } catch {
      runtime2.log(`  Could not process market #${i2} — skipping`);
    }
  }
  runtime2.log(`=== Workflow Complete: ${settledCount} markets settled ===`);
  return JSON.stringify({
    status: "success",
