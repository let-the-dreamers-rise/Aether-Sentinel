// ../../Users/ASHWIN GOYAL/OneDrive/Desktop/AETHER SENTINEL/cre-workflows/risk-monitoring-workflow/.cre_build_tmp.js
...
var configSchema = exports_external.object({
  ...
  tokenizedVaultAddress: exports_external.string(),
  riskGuardianAddress: exports_external.string(),
  aiRiskEngineUrl: exports_external.string(),
  riskThreshold: exports_external.number().default(60),
...
var fetchAIRiskAssessment = (nodeRuntime, vaultPayload) => {
...
var onCronTrigger = (runtime2) => {
  ...
  runtime2.log("=== AETHER SENTINEL Risk Monitoring Workflow ===");
  runtime2.log(`Vault: ${config.tokenizedVaultAddress}`);
  runtime2.log(`Guardian: ${config.riskGuardianAddress}`);
  ...
  runtime2.log("[Step 1] Reading vault state from TokenizedVault...");
  let reserveRatio = 5000;
  let totalDeposits = 1e6;
  ...
  try {
    ...
    const vaultStateRaw = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: config.tokenizedVaultAddress,
    ...
    if (hexData && hexData !== "0x") {
      ...
      isPaused = vaultState[3];
    } else {
      runtime2.log("  Contract returned empty data — using default vault state for demo");
    }
  } catch {
    runtime2.log("  Could not read vault state — using defaults for simulation demo");
  }
  runtime2.log(`  Reserve Ratio: ${reserveRatio} bps (${reserveRatio / 100}%)`);
  runtime2.log(`  Total Deposits: ${totalDeposits}`);
  runtime2.log(`  Total Liabilities: ${totalLiabilities}`);
  runtime2.log(`  Paused: ${isPaused}`);
  if (isPaused) {
    runtime2.log("Vault is paused — skipping risk assessment.");
    return JSON.stringify({ status: "skipped", reason: "vault_paused" });
  }
  runtime2.log("[Step 2] Calling AI Risk Engine for assessment...");
  const riskPayload = JSON.stringify({
    vault_state: {
      reserve_ratio: reserveRatio / 1e4,
  ...
  let riskScoreNum = 50;
  try {
    const riskScore = runtime2.runInNodeMode(fetchAIRiskAssessment, consensusMedianAggregation())(riskPayload).result();
    riskScoreNum = Number(riskScore);
  } catch {
    runtime2.log("  AI engine unavailable — using fail-safe risk score of 50");
  }
  runtime2.log(`  Risk Score: ${riskScoreNum}/100`);
  if (riskScoreNum >= config.riskThreshold) {
    runtime2.log(`[Step 3] Risk score ${riskScoreNum} >= threshold ${config.riskThreshold} — executing on-chain response...`);
    try {
      const reportData = encodeAbiParameters(parseAbiParameters("uint256 riskScore, uint256 confidence"), [BigInt(riskScoreNum), BigInt(80)]);
      ...
      runtime2.log(`  TX submitted: ${txHash}`);
      runtime2.log(`  View: https://sepolia.etherscan.io/tx/${txHash}`);
    } catch {
      runtime2.log("  On-chain write skipped (contract not yet deployed)");
    }
  } else {
    runtime2.log(`[Step 3] Risk score ${riskScoreNum} < threshold ${config.riskThreshold} — no on-chain action needed.`);
  }
